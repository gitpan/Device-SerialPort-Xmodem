# This Makefile is for the Device::SerialPort::Xmodem extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/Device/SerialPort/Xmodem.pm]
#     AUTHOR => q[Aaron Mitti <mitti@cpan.org>]
#     DEFINE => q[]
#     INC => q[-I.]
#     LIBS => [q[]]
#     NAME => q[Device::SerialPort::Xmodem]
#     PREREQ_PM => { Device::SerialPort=>q[0] }
#     VERSION_FROM => q[lib/Device/SerialPort/Xmodem.pm]
...
DIRFILESEP = /
DFSEP = $(DIRFILESEP)
NAME = Device::SerialPort::Xmodem
NAME_SYM = Device_SerialPort_Xmodem
VERSION = 1.00
...
FULLEXT = Device/SerialPort/Xmodem
BASEEXT = Xmodem
PARENT_NAME = Device::SerialPort
DLBASE = $(BASEEXT)
VERSION_FROM = lib/Device/SerialPort/Xmodem.pm
INC = -I.
DEFINE = 
...
H_FILES  = ppport.h
MAN1PODS = 
MAN3PODS = lib/Device/SerialPort/Xmodem.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Device/SerialPort
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Device/SerialPort
...
TO_INST_PM = lib/Device/SerialPort/Xmodem.pm

PM_TO_BLIB = lib/Device/SerialPort/Xmodem.pm \
...
DISTNAME = Device-SerialPort-Xmodem
DISTVNAME = Device-SerialPort-Xmodem-1.00
...
# Device::SerialPort::Xmodem might depend on some other libraries:
...
manifypods : pure_all  \
	lib/Device/SerialPort/Xmodem.pm \
	lib/Device/SerialPort/Xmodem.pm
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Device/SerialPort/Xmodem.pm $(INST_MAN3DIR)/Device::SerialPort::Xmodem.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:         Device-SerialPort-Xmodem' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:      1.00' >> META_new.yml
	$(NOECHO) $(ECHO) 'version_from: lib/Device/SerialPort/Xmodem.pm' >> META_new.yml
...
ppd:
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Xmodem file transfer protocol for Device::SerialPort</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Aaron Mitti &lt;mitti@cpan.org&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Device-SerialPort" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' \
	  lib/Device/SerialPort/Xmodem.pm blib/lib/Device/SerialPort/Xmodem.pm 
